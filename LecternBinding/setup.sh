...
Usage: remind [flag]
  ...
  -s Starts the script
  -t runs the unit testing environment
...
echo -e "---Python File Setup---"
while getopts "dhist" options; do #Using getopts for options, no : if no argument
	case "${options}" in                        
	    i) init "$production";;
        s) start ;;
        t) test ;;
